...
| 数据源 | 它告诉你什么 |
|--------|--------------|
| **Bilibili（B站）** | 中国的视频广场。按 B站用户真正花出去的互动排序——收藏、点赞、弹幕、播放。公开 WBI 签名搜索，零配置、免登录。 |
...
| 平台 | 状态 | 实现方式 |
|------|------|----------|
| **Bilibili（B站）** | ✅ 已上线 | 公开 WBI 签名搜索，自动获取 `buvid3` cookie，零配置。按 收藏 > 点赞 > 弹幕 > 播放 排序。 |
...
按搜索接口实际返回的互动信号加权,收藏权重最高(代表留存意图):

```
weighted = 播放×1 + 点赞×2 + 弹幕×1.5 + 收藏×2.5
```

经 log 压缩后归一化到 0.05–1.0,让一个中等热度视频落在 0.5 附近,爆款逼近 1.0。

> 注:投币是 B站最强的认可信号,但 `/search/type` 接口不返回投币数,因此暂不计入打分。投币感知排序留给后续抓取完整视频数据的 enrichment PR。
